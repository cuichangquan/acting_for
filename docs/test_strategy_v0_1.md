...
必須Security Invariantは **`require_approval != allow`**。特にstatusが `:require_approval` の場合、`allowed? == false`、`approval_required? == true` を必須Testとする。constructor等の未決定の生成方法は固定しない。
