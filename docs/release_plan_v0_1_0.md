...
6. Create / push `v0.1.0` at the pinned artifact-source commit from step 3, so all packaged contents match the tag exactly. Confirm remote tag points to that commit. Do not move / overwrite an existing tag. RubyGems is already verified, so no tag is created for an unpublished gem.
7. Create GitHub Release `v0.1.0` with the finalized body from `release_notes_v0_1_0.md` (formal publication target: GitHub Releases); include artifact-source SHA / byte size / SHA256. Verify public tag, Release, installation, and links.
8. Update README on main to factual released availability, and CURRENT_STATE / PROGRESS with actual publication evidence. No production / dependency / packaging change is permitted. The tag's packaged README remains the truthful pre-push snapshot; current main reports publication. Commit / push and wait for all five CI jobs green. Report artifact source / tagged SHA, final main SHA, RubyGems / Release URLs and checksum. Latest Decision changes only if separately approved.
