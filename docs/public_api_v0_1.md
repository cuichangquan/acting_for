...
Gem構成・配置、Rails標準Migration方式、独自Generator非提供は[Step 7の正本](gem_structure_v0_1.md)（D028）で設計決定済み。Migration / Models / revoke! / delegate / Authorization queryは後続実装Decisionで実装・検証済み。現在の実装・release状態は[CURRENT_STATE](CURRENT_STATE.md)を参照。

## 15. Step 5 Progress

Step 4は完了。Step 5「Public API Design」も **10 / 10、Complete**。後続のStep 6は **Complete / Design-stage Quick Start finalized**（D027）。最新の進捗は[PROGRESS](PROGRESS.md)、実行可能なQuick Startは[README](../README.md#quick-start)を参照。delegate / authorizeはRunnable Quick StartとOfficial Demoで検証済みで、ActingFor 0.1.0として公開済み。
