...
The future runnable Quick Start will use a supported new Rails application and proceed in order through gem installation, migration copying and execution, Agent creation, Delegation creation, authorization, and Decision inspection (D068–D071). It will use Rails console for minimal developer verification; production Agent / Delegation management remains the host application's responsibility. No dedicated controller, route, view, UI, or management screen is required for this Quick Start. It will use only supported public APIs and standard Rails operations, with no Quick Start-only API, helper, or setup API. The Shopping Agent example will remain: Principal `User`, Agent Shopping Agent, Action `purchase`, Resource `Product`, Context `amount`. These are examples, not required host models or business rules. Runnable code will be written after gem implementation; the existing design-stage example below remains non-runnable.
...
The scope document distinguishes finalized product boundaries and test acceptance criteria, the finalized overall Definition of Done, and implemented features. The authoritative Step 5 public API document distinguishes decided design from undecided candidates. The API is not implemented or published yet.
